...
const getToken = async (code) => {
  ...
  const response = await fetch(
    "https://9w6q43w6d3.execute-api.eu-central-1.amazonaws.com/dev/api/token/" +
      "/" +
      encodeCode
  );
...
export const getAccessToken = async () => {
  ...
  if (!accessToken || tokenCheck.error) {
    ...
    if (!code) {
      const response = await fetch(
        "https://9w6q43w6d3.execute-api.eu-central-1.amazonaws.com/dev/api/get-auth-url"
      );
      const result = await response.json();
...
export const getEvents = async () => {
  ...
  if (token) {
    removeQuery();
    const url =
      "https://9w6q43w6d3.execute-api.eu-central-1.amazonaws.com/dev/api/get-events" +
      "/" +
      token;
